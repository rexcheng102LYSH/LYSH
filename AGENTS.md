...
- Optimization rule (context efficiency):
  - In the same session, after one successful read, cache both files as active context.
  - For follow-up turns, do a lightweight change check first (e.g. file modified time/hash).
  - Re-read both files only when:
    1) either file changed, or
    2) session restarted/lost memory, or
    3) user explicitly asks to refresh rules/context.
  - If none of the above triggers occur, do not full-read both files again.
