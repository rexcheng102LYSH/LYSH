...
const VisualFX = {
    ...
    startCelebration: function(type) {
        ...
        // [New] 流金特效初始化
            ...
            this.state.golden = {
                ...
                // COMPLETE 文本特效
                completeText: null,
                completeTriggered: false // 【新增】标记 COMPLETE 是否已触发
            };
    ...
    updateCoins: function(ctx, w, h, now, golden) {
        ...
        if (golden.totalCoins < golden.maxCoins && now - golden.lastCoinTime > golden.coinInterval) {
            ...
            golden.coins.push({
                ...
                vy: 1.8, // 【修改】提升下落速度从 1.7 到 1.8 (+0.1)
                rotation: 0,
                rotSpeed: 0.05, // 固定旋转速度
                size: 33, // 【修改】固定大小从 30px 增加到 33px (+10%)
                life: 1.0,
                collected: false,
    ...
    renderTrailSparkles: function(ctx, coin, now) {
        ...
        coin.trail.forEach((point, index) => {
            if (index % 3 !== 0) return; // 每3个轨迹点生成一个粒子
