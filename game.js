...
function t(key, path=null) { 
    if(path) return I18N[curLangKey][path][key]; 
...
function showScreen(n) { 
    if (n !== 'settings') {
        Object.values(screens).forEach(s => { 
            if(s && s.id!=='settingsModal') s.classList.remove('active'); 
        }); 
    }
...
function enterTurnSelection(mode, diff) {
    ...
    updateStaticText(); 
    if (isBO3 && (p1Score > 0 || p2Score > 0)) { 
        tEl.innerText = t('titlePickSide'); 
        dEl.innerText = `${t('descPickSideLoser')} (${chooser==='p1'?"P1":"P2"})`; 
    } else { 
        tEl.innerText = t('titlePickSide'); 
...
function goBackFromTurn() { 
    SoundEngine.playPlace(); 
    if (gameMode === 'pve') { showScreen('diff'); } 
    else { goToMenu(); } 
}
...
function handleTurnChoice(c) { 
    SoundEngine.playPlace(); 
    if (gameMode === 'pve') { 
        humanSide = (c === 1) ? MAPLE : SUN; 
        enterDraftPhase(); 
    } else { 
        if (c === 1) { 
            playerSides[MAPLE] = chooser; playerSides[SUN] = (chooser === 'p1' ? 'p2' : 'p1'); 
        } else { 
            playerSides[SUN] = chooser; playerSides[MAPLE] = (chooser === 'p1' ? 'p2' : 'p1'); 
        } 
        enterDraftPhase(); 
    } 
}
...
function renderSkillGrid() { 
    ...
    SKILL_IDS.forEach(sid => { 
        ...
        c.innerHTML = `<div class="skill-title">${sd.name}</div><div class="skill-desc">${sd.desc}</div>`; 
        c.onclick = () => pickSkill(sid); 
        if (Object.values(playerSkills).includes(sid)) { 
            c.classList.add('selected'); c.onclick = null; 
        } 
        g.appendChild(c); 
    }); 
}
...
function pickSkill(id) { 
    SoundEngine.playPlace(); 
    playerSkills[draftTurn] = id; 
    if (draftTurn === SUN) { 
        draftTurn = MAPLE; renderSkillGrid(); updateDraftTitle(); 
    } else {
        initGame(); 
    }
}
...
function initGame() {
    ...
    if(gameMode === 'pve') { 
        dt.style.display = 'inline-block'; sb.style.display = 'none'; 
        dt.innerText = t('pveTag') + t('diff' + aiDifficulty.charAt(0).toUpperCase() + aiDifficulty.slice(1)); 
    } else { 
        dt.style.display = 'none'; sb.style.display = isBO3 ? 'block' : 'none'; 
        updateScoreBoard(); 
    }
    renderBoard();
...
function restoreState(state) { 
    ...
    territoryZones = state.territoryZones; 
    chaosDebuff = state.chaosDebuff; 
    ...
    for(let r=0; r<BOARD_SIZE; r++) for(let c=0; c<BOARD_SIZE; c++) { 
        ...
        if (val === MAPLE || val === SUN) { 
            const pc = document.createElement('span'); pc.className='piece'; pc.innerText=ICONS[val]; cell.appendChild(pc); 
...
function placePiece(r, c, p, m=false, chaos=false) { 
    ...
    const cell = getCell(r,c); 
    if(cell) { 
        const pc = document.createElement('span'); pc.className='piece'; pc.innerText=ICONS[p]; cell.appendChild(pc); 
        SoundEngine.playPlace(); 
    } 
}
...
function handleCellHover(r, c) { 
    if (activeEffect === 'zone_pick') { 
        ...
        for(let i=r-1; i<=r+1; i++) for(let j=c-1; j<=c+1; j++) 
            if(isValid(i,j)) { const el=getCell(i,j); if(el) el.classList.add('territory-preview'); } 
    } 
}

function checkWinAndSwitch(r, c, p) { if (checkWin(r, c, p)) handleMatchEnd(p); else switchTurn(); }

// --- 核心逻辑：SwitchTurn 控制 BGM ---
function switchTurn() {
    territoryZones.forEach(z => { if(z.owner===currentPlayer) z.turns--; }); 
    ...
    // BGM Logic: 
    // 1. 如果当前玩家是炸弹受害者 -> 播放 Bomb Music
    // 2. 如果不是 -> 播放用户选择的 Music
    const userPref = document.getElementById('trackOverture').classList.contains('active') ? 'overture' : 'origin';
...
function handleSkillInteraction(r, c) {
    ...
    else if (activeEffect === 'move_drop') { if (board[r][c]!==EMPTY || isZoneRestricted(r,c,currentPlayer)) { SoundEngine.playError(); return; } const src = effectData.src; board[src.r][src.c] = EMPTY; const sc = getCell(src.r, src.c); if(sc){sc.innerHTML=''; sc.style.opacity='1';} placePiece(r, c, src.val, true); activeEffect = null; b.classList.remove('casting-move-dest'); if (checkWin(r, c, src.val)) handleMatchEnd(src.val); else showToast(t('moveDone', 'toast')); } 
    ...
    else if (activeEffect === 'swap_pick_2') { const p = board[r][c]; const enemy = currentPlayer===MAPLE?SUN:MAPLE; if (p!==enemy) { SoundEngine.playError(); return; } const s1 = effectData.swapSrc; const s2 = {r, c, val: p}; const c1 = getCell(s1.r, s1.c); if(c1) c1.style.opacity = '1'; board[s1.r][s1.c] = s2.val; board[s2.r][s2.c] = s1.val; if(c1) { c1.innerHTML=''; const pc=document.createElement('span'); pc.className='piece'; pc.innerText=ICONS[s2.val]; c1.appendChild(pc); } const c2 = getCell(s2.r, s2.c); if(c2) { c2.innerHTML=''; const pc=document.createElement('span'); pc.className='piece'; pc.innerText=ICONS[s1.val]; c2.appendChild(pc); } activeEffect = null; b.classList.remove('casting-move-dest'); if(checkWin(s1.r, s1.c, s2.val)) { handleMatchEnd(s2.val); return; } if(checkWin(s2.r, s2.c, s1.val)) { handleMatchEnd(s1.val); return; } showToast(t('swapDone', 'toast')); }
}
...
function getScoreMove(adv, mas=false) { 
    let max=-Infinity, ms=[]; 
    const ai=currentPlayer, hum=currentPlayer===MAPLE?SUN:MAPLE; 
    for(let r=0;r<BOARD_SIZE;r++) for(let c=0;c<BOARD_SIZE;c++) { 
        ...
        if(!adv) s=a+d; 
            ...
            s=a*(mas?1.2:1)+d; 
            if(a>=1e5)s=Infinity; else if(d>=1e5)s=9e7; else if(a>=1e4)s+=5e4; else if(d>=1e4)s+=4e4; 
        } 
        s+=Math.random()*10; 
        if(s>max){max=s;ms=[{r,c}]} else if(Math.abs(s-max)<5)ms.push({r,c}); 
    } 
...
function checkWin(r,c,p){ const d=[[0,1],[1,0],[1,1],[1,-1]]; const limit = shortBattleTurns > 0 ? 4 : 5; for(let k of d){let ct=1; for(let s of [1,-1]){let i=r,j=c;while(true){i+=k[0]*s;j+=k[1]*s;if(!isValid(i,j)||board[i][j]!==p)break;ct++}} if(ct>=limit)return true;} return false; }
...
function updateDynamicUI(){
    ...
    if (bombTarget === MAPLE) { if (timeRemaining[MAPLE] < 30) t1.classList.add('timer-critical'); else t1.classList.add('timer-bomb'); } else if (timeRemaining[MAPLE] < 30) { t1.classList.add('danger'); }
    if (bombTarget === SUN) { if (timeRemaining[SUN] < 30) t2.classList.add('timer-critical'); else t2.classList.add('timer-bomb'); } else if (timeRemaining[SUN] < 30) { t2.classList.add('danger'); }
